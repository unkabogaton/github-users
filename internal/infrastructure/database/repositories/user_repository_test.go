...
var sampleUser *entities.User = &entities.User{
	ID:           1,
...
func TestUserRepository_List_WithPaginationAndOrdering(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "login", "node_id", "avatar_url", "url", "html_url", "type", "user_view_type", "site_admin", "updated_at", "created_at"}).
		AddRow(
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, login, node_id, avatar_url, url, html_url, type, user_view_type, site_admin, updated_at, created_at FROM github_users ORDER BY login ASC LIMIT $1 OFFSET $2")).
		WithArgs(5, 5).
		WillReturnRows(rows)

	options := interfaces.ListOptions{Limit: 5, Page: 2, OrderBy: "login", OrderDirection: "ASC"}
	users, listErr := repository.List(context.Background(), options)
...
func TestUserRepository_DeleteByLogin(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(
		`DELETE FROM github_users WHERE login = $1`,
	)).
